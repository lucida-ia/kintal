...
export async function GET(request: NextRequest) {
  try {
    ...
    const errorTypes = events.reduce(
      (acc: Record<string, number>, event: PostHogEvent) => {
        const properties = event.properties || {};
    ...
    const hourlyData = events.reduce(
      (acc: Record<string, HourlyDataPoint>, event: PostHogEvent) => {
        const properties = event.properties || {};
        const errorTypesArray = Array.isArray(properties.$exception_types)
          ? properties.$exception_types
          : [];
    ...
    const chartData = (Object.values(hourlyData) as HourlyDataPoint[]).sort(
      (a: HourlyDataPoint, b: HourlyDataPoint) => a.time.localeCompare(b.time)
    );
    ...
    const weeklyUrl = `https://us.posthog.com/api/projects/${POSTHOG_PROJECT_ID}/events/?event=$exception&after=${oneWeekAgo.toISOString()}`;
